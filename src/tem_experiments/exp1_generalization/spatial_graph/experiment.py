"""Full training + evaluation loop for Spatial Graph."""
...
from tem_experiments.exp1_generalization.spatial_graph.definitions import (
    EVAL_EPISODES,
    EVAL_EXPLORE_LEN,
    HEIGHT,
    WIDTH,
...
def run_spatial_graph(
    ...
    print(
        f"Spatial Graph experiment — {num_steps} steps, "
        f"{height}x{width}, device={device}"
    )
    t0 = time.time()

    for step in tqdm(range(1, num_steps + 1), desc="Spatial Graph", unit="step"):
        model.train()
